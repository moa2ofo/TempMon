...
#else
static long long CMock_Guts_Space[(CMOCK_MEM_SIZE + CMOCK_MEM_ALIGN_SIZE +
                                   sizeof(long long) - 1) /
                                  sizeof(long long)];
...
CMOCK_MEM_INDEX_TYPE CMock_Guts_MemNew(CMOCK_MEM_INDEX_TYPE size) {
  ...
  if ((CMock_Guts_BufferSize - CMock_Guts_FreePtr) < size) {
    ...
    return CMOCK_GUTS_NONE; /* nothing we can do; our static buffer is out of
                               memory */
#else
...
CMOCK_MEM_INDEX_TYPE CMock_Guts_MemChain(CMOCK_MEM_INDEX_TYPE root_index,
  ...
  if (root_index == CMOCK_GUTS_NONE) {
    /* if there is no root currently, we return this object as the root of the
     * chain */
    return obj_index;
  } else {
...
CMock_Guts_MemNext(CMOCK_MEM_INDEX_TYPE previous_item_index) {
  CMOCK_MEM_INDEX_TYPE index;
  void *previous_item;
