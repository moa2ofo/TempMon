...
/* Reference CRC-8 table values (pre-calculated for verification with polynomial
 * 0x07u) */
...
void test_BuildTbl_PopulatesTable(void) {
  ...
  TEST_ASSERT_EQUAL_INT(0, tableEmpty);
}

/* Test that specific entries match expected CRC-8 values */
void test_BuildTbl_CorrectValues(void) {
  BuildTbl();

  /* Test first value */
  TEST_ASSERT_EQUAL_HEX8(expected_crc8_table[0], Crc8Tbl_u8[0]);

  /* Test middle values */
  TEST_ASSERT_EQUAL_HEX8(expected_crc8_table[128], Crc8Tbl_u8[128]);

  /* Test last value */
  TEST_ASSERT_EQUAL_HEX8(expected_crc8_table[255], Crc8Tbl_u8[255]);
}

/* Test that the entire table matches the expected values */
void test_BuildTbl_EntireTableCorrect(void) {
  BuildTbl();

  for (uint16_t i = 0u; i < 256u; i++) {
    TEST_ASSERT_EQUAL_HEX8_MESSAGE(expected_crc8_table[i], Crc8Tbl_u8[i],
                                   "Table mismatch at index");
  }
}

/* Test specific known CRC-8 values */
void test_BuildTbl_SpecificValues(void) {
  BuildTbl();

  /* Test a few specific entries */
  TEST_ASSERT_EQUAL_HEX8(0x07, Crc8Tbl_u8[1]);
  TEST_ASSERT_EQUAL_HEX8(0x0E, Crc8Tbl_u8[2]);
  TEST_ASSERT_EQUAL_HEX8(0x09, Crc8Tbl_u8[3]);
  TEST_ASSERT_EQUAL_HEX8(0x1C, Crc8Tbl_u8[4]);
}

/* Test that table entries follow the expected pattern based on algorithm */
void test_BuildTbl_AlgorithmPattern(void) {
  BuildTbl();

  /* First 16 values should be deterministic based on algorithm with polynomial
   * 0x07u */
  uint8_t expectedFirst16[] = {0x00, 0x07, 0x0E, 0x09, 0x1C, 0x1B, 0x12, 0x15,
                               0x38, 0x3F, 0x36, 0x31, 0x24, 0x23, 0x2A, 0x2D};

  for (uint8_t i = 0u; i < 16u; i++) {
    TEST_ASSERT_EQUAL_HEX8(expectedFirst16[i], Crc8Tbl_u8[i]);
  }
}

/* Test that the polynomial constant is used correctly */
void test_BuildTbl_PolynomialDefined(void) {
  /* Verify the polynomial is defined and non-zero */
  TEST_ASSERT_NOT_EQUAL(0, CRC8_POLY_U8);
  TEST_ASSERT_EQUAL_HEX8(0x07, CRC8_POLY_U8);
}
